...
def main():
    ...
    try:
        ...
        # 터미널을 Ctrl+C 한 번으로 여기 finally까지 왔는데, run_explore.py의 종료 절차가
        # 각 자식 프로세스에 SIGINT를 한 번 더 보내는 것과 겹치면(터미널 자체가 보낸 SIGINT +
        # run_explore.py가 보낸 SIGINT), 이 정리 코드가 termios 복구 전에 두 번째 인터럽트로
        # 끊겨서 터미널이 raw 모드(에코 없음)로 고정돼버리는 문제가 있었다. 복구가 끝날 때까지만
        # SIGINT를 무시해서 반드시 원래 터미널 설정으로 되돌아가게 한다.
