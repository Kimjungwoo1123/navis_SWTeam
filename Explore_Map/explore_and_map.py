...
[탐색 중 조향/속도 제한]
탐색 단계는 처음 가보는 실내 공간을 도는 만큼 조향은 RC카 전체 한계(MAX_STEERING_DEG=30도)의
1/3(EXPLORE_MAX_STEERING_DEG=10도)로 제한합니다. 속도는 원래 같이 1/3로 제한했었는데, 실기에서
그 정도로는 정체(stall) 감지 기준을 채울 만큼도 못 움직여서 오탐(실제론 그냥 느린 것뿐인데
"걸렸다"고 판단)이 계속 나 전체 출력(CRUISE_SPEED_PERCENT_EXPLORE=100%)까지 쓰도록 풀었습니다.
...
# 탐색 단계는 처음 가보는 실내 공간을 도는 만큼 조향은 안전하게 RC카 전체 한계의 1/3로 제한한다.
# 속도는 원래 이것도 1/3(약 33%)로 제한했었는데, apply_min_moving_floor의 데드존(60%) 보정을
# 거쳐도 여전히 정체(stall) 감지 기준(STALL_DISPLACEMENT_MM/STALL_CHECK_WINDOW_S)을 채울 만큼
# 못 움직여서 "실제로는 그냥 느리게 가고 있을 뿐"인데 "걸려서 멈췄다"고 오판, 후진이 자꾸
# 발동하는 문제가 실기에서 나왔다 (2026-07-11). 그래서 속도는 전체 출력(100%)까지 쓰도록 풀었다.
EXPLORE_STEERING_SCALE = 1.0 / 3.0
EXPLORE_MAX_STEERING_DEG = MAX_STEERING_DEG * EXPLORE_STEERING_SCALE  # 10.0도
