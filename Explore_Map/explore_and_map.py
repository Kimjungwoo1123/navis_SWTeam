...
[탐색 중 조향/속도 제한]
탐색 단계는 처음 가보는 실내 공간을 도는 만큼, 조향/속도 모두 RC카 전체 한계
(MAX_STEERING_DEG=30도, speed_percent 100%)의 1/3 수준(EXPLORE_SCALE)으로만 사용해서
안전하게 저속으로 돕니다 - EXPLORE_MAX_STEERING_DEG(10도), CRUISE_SPEED_PERCENT_EXPLORE(약 33%).
...
# 탐색 단계는 처음 가보는 실내 공간을 도는 만큼, 조향/속도 모두 RC카 전체 한계의 1/3 수준으로만
# 사용해서 안전하게 저속으로 탐색한다.
EXPLORE_SCALE = 1.0 / 3.0
